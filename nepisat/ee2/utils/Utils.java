...
{	
		...
		try 
		{
			...
			if (ent instanceof EntitySkeleton)
			{
				if (world.rand.nextInt(2) == 0)
				{
					...
					ent.setCurrentItemOrArmor(0, new ItemStack(Item.swordStone));
				}
				else 
				{
					ent.setCurrentItemOrArmor(0, new ItemStack(Item.bow));
				}
			}
			else if (ent instanceof EntityPigZombie)
			{
				ent.setCurrentItemOrArmor(0, new ItemStack(Item.swordGold));
			}
	...
	{
		int oreIds = OreDictionary.getOreID(stack);
		...
		return OreDictionary.getOreName(oreIds);
	}
	...
	public static ArrayList<ItemStack> getBlockDrops(World world, EntityPlayer player, Block block, ItemStack stack, int x, int y, int z)
	{
		...
		return block.getBlockDropped(world, x, y, z, meta, EnchantmentHelper.getEnchantmentLevel(Enchantment.fortune.effectId, stack));
	}
