package nepisat.ee2.playerData;
...
public final class IOHandler
{
	...
	private static void readData()
	{
		...
		if (knowledge != null)
		{
			NBTTagList tomeKnowledge = knowledge.getTagList("Tome Knowledge");
			
			for (int i = 0; i < tomeKnowledge.tagCount(); i++)
			{
				NBTTagCompound tag = (NBTTagCompound) tomeKnowledge.tagAt(i);
			...
			NBTTagList list = knowledge.getTagList("knowledge");
			...
			for (int i = 0; i < list.tagCount(); i++)
			{
				...
				NBTTagList subList = subTag.getTagList("data");
				
				for (int j = 0; j < subList.tagCount(); j++)
				{
					ItemStack stack = ItemStack.loadItemStackFromNBT((NBTTagCompound) subList.tagAt(j));
			...
			NBTTagList emc = knowledge.getTagList("playerEMC");
			
			for (int i = 0; i < emc.tagCount(); i++)
			{
				NBTTagCompound tag = (NBTTagCompound) emc.tagAt(i);
		...
		if (bagData != null)
		{
			NBTTagList list = bagData.getTagList("bagdata");
			
			for (int i = 0; i < list.tagCount(); i++)
			{
				NBTTagCompound nbt = (NBTTagCompound) list.tagAt(i);
				
				NBTTagList subList = nbt.getTagList("data");
				
				for (int j = 0; j < subList.tagCount(); j++)
				{
					NBTTagCompound subNbt = (NBTTagCompound) subList.tagAt(j);
					
					ItemStack[] inv = new ItemStack[104];
					
					NBTTagList subList2 = subNbt.getTagList("inv");
					
					for (int k = 0; k < subList2.tagCount(); k++)
					{
						NBTTagCompound subNbt2 = (NBTTagCompound) subList2.tagAt(k);
