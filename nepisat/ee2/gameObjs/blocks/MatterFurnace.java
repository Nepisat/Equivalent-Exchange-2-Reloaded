...
public class MatterFurnace extends BlockDirection implements ITileEntityProvider
{
	...
	public MatterFurnace(int i,boolean active, boolean isRM) 
	{
	...
	@Override
	public int idDropped(int no, Random rand, int clue)
	{
		if (isHighTier)
		{
			return ObjHandler.rmFurnaceOff.blockID;
		}
		
		return ObjHandler.dmFurnaceOff.blockID;
	}
	...
	public void updateFurnaceBlockState(boolean isActive, World world, int x, int y, int z)
	{
		...
		if (isActive)
		{
			...
			else world.setBlock(x, y, z, ObjHandler.dmFurnaceOn.blockID);
		}
		else
		{
			if (isHighTier)
				world.setBlock(x, y, z, ObjHandler.rmFurnaceOff.blockID);
			else world.setBlock(x, y, z, ObjHandler.dmFurnaceOff.blockID);
		}
