...
public class EntityWaterProjectile extends EntityThrowable
{
			...
			for (int x = (int) (this.posX - 3); x <= this.posX + 3; x++)
				for (int y = (int) (this.posY - 3); y <= this.posY + 3; y++)
					for (int z = (int) (this.posZ - 3); z <= this.posZ + 3; z++)
					{
						...
						if (block == Block.lavaStill)
						{
							this.worldObj.setBlock(x, y, z, Block.obsidian.blockID);
						}
						else if (block == Block.lavaMoving)
						{
	...
	protected void onImpact(MovingObjectPosition mop) 
	{
		if (this.worldObj.isRemote)
		{
			...
			this.worldObj.setBlock(mop.blockX + dir.offsetX, mop.blockY + dir.offsetY, mop.blockZ + dir.offsetZ, Block.waterMoving.blockID);
			this.setDead();
